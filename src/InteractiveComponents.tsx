...
export interface InteractiveQuestion {
  type: 'numeric' | 'highlight-text' | 'highlight-findings' | 'matrix' | 'dropdown' | 'diagram-click' | 'drag-categories';
  // For numeric
  unit?: string;
  hint?: string;
  // For highlight-text
...
// ===== NUMERIC INPUT =====
export function NumericInput({ question, value, onChange }: {
...
// ===== HIGHLIGHT TEXT =====
export function HighlightText({ question, value, onChange }: {
  question: InteractiveQuestion;
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-gray-500 mb-2">Click on the text to highlight relevant findings:</p>
...
// ===== HIGHLIGHT FINDINGS =====
export function HighlightFindings({ question, value, onChange }: {
  question: InteractiveQuestion;
  value: number[];
  onChange: (val: number[]) => void;
}) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-cyan-400 mb-2">Click findings that warrant follow-up / should be highlighted:</p>
      <div className="space-y-2">
        {question.findings?.map((finding, idx) => {
          ...
          return (
            ...
            >
              <span className={`text-sm ${isHighlighted ? 'text-yellow-200 font-semibold' : 'text-gray-300'}`}>
                {finding}
              </span>
            </button>
          );
...
// ===== MATRIX GRID =====
export function MatrixGrid({ question, value, onChange }: {
  question: InteractiveQuestion;
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-cyan-400 mb-2">
        For each row, select the matching category. Each row must have one selection.
      </p>
          ...
          <thead>
            <tr>
              ...
              {question.columns?.map((col, ci) => (
                <th key={ci} className="border border-gray-700 p-2 text-center bg-gray-800 text-gray-300 font-medium text-xs min-w-[80px]">
                  {col}
                </th>
              ))}
            </tr>
...
// ===== DROPDOWN FILL =====
export function DropdownFill({ question, value, onChange }: {
  question: InteractiveQuestion;
        ...
        <div key={bi} className="space-y-2">
          <p className="text-sm text-gray-300">
            ...
            <span className="inline-block mx-1">
              <select
                value={value[bi] ?? -1}
                onChange={(e) => onChange({ ...value, [bi]: parseInt(e.target.value) })}
...
// ===== DIAGRAM CLICK =====
export function DiagramClick({ question, value, onChange }: {
  question: InteractiveQuestion;
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-cyan-400 mb-2">
        Click the location on the diagram that corresponds to the finding:
      </p>
      {question.diagramDescription && (
        <p className="text-sm text-gray-400 mb-3 italic">{question.diagramDescription}</p>
      )}
      <div className="grid grid-cols-2 gap-3">
        {question.locations?.map((loc, idx) => {
          ...
          return (
            ...
            >
              <div className={`text-lg font-bold mb-1 ${isSelected ? 'text-emerald-400' : 'text-gray-400'}`}>
                {loc.label}
              </div>
...
export function DragCategories({ question, value, onChange }: {
  question: InteractiveQuestion;
    ...
    <div className="space-y-4">
      ...
      <div className="p-3 rounded-xl bg-gray-800/30 border border-gray-700">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {unassigned.length === 0 && (
            <span className="text-xs text-gray-600 italic">All items assigned</span>
          )}
        </div>
      </div>
          ...
          >
            ...
            <div className="flex flex-wrap gap-1.5">
              {(value[cat.name] || []).map((item, idx) => (
                <span
                  key={idx}
                  onClick={(e) => { e.stopPropagation(); removeFromCategory(cat.name, item); }}
                ...
                >
                  {item}
                  <button className="text-emerald-400/60 hover:text-red-400">×</button>
                </span>
              ))}
            </div>
          </div>
